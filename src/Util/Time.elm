...
durationBetween t1 t2 =
    ...
    if diff <= 0 then
        ...
        let
            ...
            remainingAfterHours =
                remainingAfterDays - hours * 60 * 60

            minutes = 
                remainingAfterHours // 60

            seconds =
                remainingAfterHours - minutes * 60
        in
        Just
            { seconds = seconds
            , minutes = minutes
    ...
    let
        parts =
            [ if duration.days > 0 then
                    Just (String.fromInt duration.days ++ " day" ++ (if duration.days == 1 then "" else "s"))
              else
                    Nothing
            , if duration.hours > 0 then
                    Just (String.fromInt duration.hours ++ " hour" ++ (if duration.hours == 1 then "" else "s"))
              else
                    Nothing
            , if duration.minutes > 0 then
                    Just (String.fromInt duration.minutes ++ " minute" ++ (if duration.minutes == 1 then "" else "s"))
              else
                    Nothing
            , if duration.seconds > 0 && duration.days == 0 && duration.hours == 0 then
                    Just (String.fromInt duration.seconds ++ " second" ++ (if duration.seconds == 1 then "" else "s"))
              else
                    Nothing
            ]

        filteredParts =
            parts |> List.filterMap identity
    in
